...
// ============================================================
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        try
        {
            logger.LogInformation("[Database] Applying relational database migrations...");
            dbContext.Database.Migrate();

            logger.LogInformation("[Program] Starting seeding data (if necessary)...");
            // Gọi hàm Seed Data của bạn ở đây (ví dụ: await DbInitializer.SeedAsync(dbContext);)
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "An error occurred while migrating or seeding the database.");
            throw; // Ném lại exception để app ngắt kết nối ngay nếu DB gặp lỗi
        }
